...
async function deleteEvent(id) {
  ...
  const token = localStorage.getItem("token");
  try {
    const res = await fetch(`https://projek-be-559917148272.us-central1.run.app/event/${id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`
      }
...
async function loadRegistrants(eventId) {
  ...
  try {
    const res = await fetch(`https://projek-be-559917148272.us-central1.run.app/registrants/${eventId}`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
